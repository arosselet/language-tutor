# Modality: Diagnosis — the tutor heals the tools, not the soul

> **Read by:** the tutor, *periodically* — not every session. Run it when the feedback ledger has
...
> **Captures:** `python scripts/sync_state.py feedback "<what the learner reacted to>"` — log it the moment
> it's said; diagnosis comes later, across the accumulation.

This is the tool that lets the tutor *grow* — and the one most able to bloat the system if it runs
loose. The whole discipline: **a missing or constraining tool is a bug to fix; never paper over a
tool gap with personality. The tutor proposes; they do not unilaterally mutate the machine.**
...
1. **Turn a dial.** Most feedback is *calibration, not a new skill*: adjust a parameter in
   ...
   Reversible. The tutor does this themselves. This is the common case — ~90% of healing.
...
3. **Propose (gated).** Only for a real structural gap: write a specific proposal **plus the
   evidence** to `docs/feature_inbox.md` for the learner's yes/no. Never self-build structure. Rare.
