...
Run `python scripts/suggest_targets.py` and read the **SCENE SPEC** block at the top. Python — not taste — owns anti-sameness: it reads the last 3 `*.tags.json` sidecars and hands you three axes already forced to diverge from recent episodes:

- **Register** — the emotional tone (`tenderness | dread | mischief | pride | suspicion | grief/nostalgia | delight | embarrassment | defiance | reconciliation`). This is the axis feeds collapse onto "mild irritation." **Honor it** — it is a gate, not a suggestion.
...
The spec guarantees range; you write the story inside it. You may still glance at the recent sidecars for `location_class` to vary terrain, but **do not override the register/form/ingredient the spec hands you** — overriding by taste is exactly how sameness drifts back.
...
`street | market | transit | restaurant | kitchen | home_social | office | extended_family | other`
...
the soak order (`"form": "narrated_drama"`, `"scale": "long"`) when the week is ready for
a batch soak; the spec's form is overridden for that episode only. It is the **batch-soak
...
minutes, not density. Structure and narrator craft: `architect.md` → The Episode Form.
`classic` (Intercept + full Breakdown) | `vignette` (Intercept only, no Breakdown — trust the scene) | `story` (one host carries a short told tale; light or no Breakdown) | `phone_call` (naturalistic call; light Breakdown) | `lore` (the analysts lead end-to-end; the payload word is the protagonist — history, kinship, myth, culture; 1–2 words told deep)

---

## Step 3: Pick the Payload

**First, read the soak order.** At the end of a session the tutor writes `soak_order` into `progress/learner.json` — the specific payload they want this episode to soak (`payload`) plus a one-line `scene_seed`. Build from it. (The tutor shapes the episode through the soak order but never appears in the audio — fourth wall per `protocol/studio/hosts.md`.) Then widen using the production axis in `progress/lexicon.json` (`cold` / `hinted` / `none`) and the `last_debrief` thread (also in `learner.json`):
- **Fired `cold` recently** → *consolidate.* Let these appear in rich, fast, natural contexts — a reward. The learner owns them; let them catch them in the wild.
...
- **The `scene_seed` / `last_debrief` thread** → echo the situation the tutor just played in chat. Tuesday's fumble becomes Thursday's scene. (Echo the *situation*, not the lesson — the hosts don't know there's a learner.)

This is what makes an episode feel alive — it tracks the learner's real week, not an abstract coverage counter.

For everything beyond the soak order, **run `python scripts/suggest_targets.py` — the same session ticket the tutor reads.** It computes the candidate set so you don't re-derive it by eye: floor-gap words, due callbacks, and priority-1 NEW candidates grouped by cluster coverage (already deduped against `progress/lexicon.json`). **Prefer priority 1 until the floor is solid;** a priority-2 word that fits the scene naturally is fine, but don't scatter into expansion while the foundation has gaps.
...
**NEW (4–5 words or phrases):** Fresh items the learner hasn't met — from the ticket's *new candidates*. Pick a **thin cluster** (the ticket shows per-cluster coverage) and take the highest-frequency, most daily-life-relevant items in it. Phrases and chunks count as items — prefer them when they're more useful than the sum of their parts (track a phrase as its own item only when it's salient as a unit, not merely compositional).

**CALLBACKS (3–5 words):** The ticket's *due callbacks* section — recognized words going stale, biased toward the floor gap; struggled words are excluded (they belong in the tutor's interactive drills, not another soak). Treat them as a **soft target: aim to land 2–3; the script leads and the quota follows.** Don't bend the scene to force every one — a callback that won't fit naturally waits (the staleness interval prevents over-rotation).
...
If the fence is empty (cold-start), note that in the brief — the Architect must scaffold heavily with the native language until the floor has words in it.
...
**NEW (X words):**
- **[target word]** (*chat form*) — definition
- ...

**CALLBACKS (X words):**
- **[target word]** (*chat form*) — definition [struggled | overdue | recently-mastered]
- ...

## Vocabulary Fence (the sea — build from these)
[Copy the full fence from suggest_targets.py output. Every target word
