...
- **Fifth milestone re-sync — selection/ledger overhaul, carried in at `template-v5-source`** (2026-07-27). The selection defects this fixed were already present in the template, not merely absent: anti-coverage sort terms, hand-copied sort keys that had silently diverged, no lifetime rep counter, no focus cohort, no exposure accounting. Seven settled conclusions follow.
- **One shared ordering law; selectors prefix, never reorder** (2026-07-27). Two separate sort keys in two files drift apart — the law gets extended in one caller and silently regresses in the other. `coverage_key()` is defined once and read by every selector; callers may prefix their own terms (the deck prefixes tier, then ask cooldown) but may not reorder or drop. Replaces the deck and floor's hand-copied sort keys, which had already diverged between `deck_status` and `floor_gap_targets`.
- **Two budgets on the floor: focus (drill) and background (expose)** (2026-07-27). Coverage and depth are in real tension — one ranked list either touches the whole population once a month (breadth, nothing graduates) or hammers a dozen (depth, the tail rots). `floor_gap_targets()` now returns `(focus, background)`; the focus set is ≤`focus_size` words in dense rotation, drilled until production goes cold; the background is exposure-only. Replaces the single ranked list that tried to do both and did neither.
- **The ledger counts declared events, never text forensics** (2026-07-27). A rep is the learner producing the word (declared by sessions and the reply judge); an exposure is delivery out the door (declared at each seam: knock push, queue drain, drill render, episode registration); a spend is an unanswered ask that feeds the cooldown and nothing else. Counting the tutor's own prose as learner production allocates focus seats by narration frequency and can flag never-drilled words as stuck. Replaces text mining as a substitute for declared writes at the delivery seam.
- **The focus cohort is stored membership, not an emergent sort** (2026-07-27). A word enters the cohort when a seat opens and leaves only on graduation; membership is a fact readable in `learner.json → focus_cohort`, immune to counting bugs by construction. Deriving membership from counters each pass would shuffle the cohort mid-flight whenever a count changed. Replaces any re-sort-on-read alternative.
- **Exposure is declared at the delivery seam, not inferred later** (2026-07-27). Every channel that puts an item in front of the learner owns the write: knock push, queue drain, drill render, episode registration. Deferring the write to a later inference pass means a channel can silently stop counting — the exposure meter goes structurally blind to whole lanes. Replaces post-hoc exposure inference.
- **Deck and floor share one ledger and one ordering law; they differ only in dosing and promotion** (2026-07-27). Both read `reps`, `exposures`, and `last_surfaced` from the same lexicon records and sort by `coverage_key`. Giving the deck its own counters would let the same word carry two different exposure counts depending on who was asking. Replaces any proposal to separate ledgers by selector.
- **Deck coverage is a distinct meter from deck progress** (2026-07-27). `deck_status` counts how many items fire cold (pace); `deck_coverage` counts how many have been worked at all (distribution). A value-ordered queue starves its tail silently: the headline can read "on pace" while the registers that carry the real stakes have never been worked. Coverage lives beside the progress headline or it is invisible. Replaces the assumption that cold/total is a sufficient picture.
