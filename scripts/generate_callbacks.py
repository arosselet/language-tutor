...
payload for the next episode — that's the soak order the tutor writes from a session.
...
Selection:
  - Pool = recognized words (comfortable + solid). Struggled words are EXCLUDED:
    ...
    the tutor's interactive modalities (roleplay, drill), not another podcast callback.
  ...
  - Cold start: on a fresh lexicon most `last_surfaced` are null, so selection
    falls back to floor-gap-first. As the tutor logs sessions the dates accrue
    and this becomes genuine spaced repetition.
