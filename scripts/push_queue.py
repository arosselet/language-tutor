...
The knock system decides WHETHER to reach out (agentic, periodic ticks); this
queue delivers pushes the tutor already decided on, at a chosen TIME — "ping me
in an hour", "field-mission debrief at 9pm", "deck rep tomorrow 8:15". Entries
are fully composed at add-time (no LLM at fire-time), live in
progress/push_queue.json on main, and are drained by a cheap CI tick
(.github/workflows/push-queue.yml, every 30 min) or by any local drain —
whoever gets there first; the queue is the single source of truth.
...
on the first tick after waking hours begin. --force marks a learner-requested
ping that fires whenever it's due (the rails protect the learner from
UNrequested pushes, not requested ones).

  python scripts/push_queue.py add --in 60 --body "eaten yet? reply in the language" \
      --expected-target "<lexicon key>" [--force]
...
Secrets: PUSH_WEBHOOK_URL (delivery). No LLM key needed.
"""
import argparse
import json
import sys
...
def cmd_drain(args):
    ...
    for e in queue:
        ...
        if not e.get("force") and non_forced_fired:
            ...
            # LAST logged fire); since knock_id was added, replies carry correlation,
            # so what remains is pacing — two doses landing in the same minute reads
            # as spam, and the next tick is ≤30 min away.
            kept.append(e)
...
def main():
    ...
    add.add_argument("--target-revealed", action="store_true",
                     help="the body shows that target text (reply caps at hinted)")
